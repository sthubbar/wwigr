> **SUPERSEDED (2026-05-30).** This file describes the original Quarto build-and-upload workflow, which is no longer used. The live site is now generated by `publish.py` (pure Python, no Quarto) and deployed by pushing to GitHub `main`, where Cloudflare Workers Builds auto-deploys. Current deploy instructions live in `README.md` (this same folder); current project state lives in `../WWIGR_HANDOFF.md`. Kept below for history only.
